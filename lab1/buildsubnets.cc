...
void End2DPlotGraph(){
	...
	plotDpPR->AddDataset(datasetDpPR);
  	...
  	ofstream plotFileRxDR("scratch/lab1/log/datarate_recv.plot"); 
	ofstream plotFileRxPR("scratch/lab1/log/pktrate_recv.plot");

  	ofstream plotFileDpDR("scratch/lab1/log/datarate_drop.plot"); 
	ofstream plotFileDpPR("scratch/lab1/log/pktrate_drop.plot");
...
int subnet_first(const int subnets, const string sendrate, uint32_t queuesize){
	...
	//globalRouting.PopulateRoutingTables();
   	Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("scratch/lab1/log/sim_subnet_queue.route", ios::out);
	...
	csma.EnableAsciiAll (ascii.CreateFileStream ("scratch/lab1/log/sim_subnet_queue.trace"));
	csma.EnablePcapAll ("scratch/lab1/log/sim_subnet_queue",true);
